...
• Incorporation of some Restful stuffs(again the core framework elements), including one of the major strategic DDD pattern of  
  "Event Driven Rest" which is one of the best Integration options for scenarios where pure HTTP is used for Integration and where Eventual Consistency rather than Transaction consistency should be fine.
...
• Will use SQL Express for integration testing of Transaction Management.SQL CE and SQL Lite seems to have lot of issues in this  
  respect (alongwith not supporting DateTimeOffset). Don't want to spend much time on SQL CE/Lite just for testing purpose.
