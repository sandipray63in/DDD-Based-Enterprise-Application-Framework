...
{
    /// <summary>
    /// Although .NET Framework's ContractUtility.Requires should suffice in most of the cases 
    /// but it has the limitations that it cannot be applied on Class variables/properties and
    /// method local variables(can only be applied on method parameters).Also all ContractUtility.Requires 
